#!/usr/bin/env python3
"""
달러인덱스(DXY) vs S&P 500 상관관계
- DXY 데이터   : yfinance (DX-Y.NYB, 일별)
- S&P 500 데이터: yfinance (^GSPC, 일별)
- 출력: docs/dxy_data.json + docs/dxy_sp500_correlation.png
"""
...
def fetch_dxy(start_date: str = START_DATE) -> pd.DataFrame:
    df = yf.download("DX-Y.NYB", start=start_date, auto_adjust=True, progress=False)
...
def draw_chart(merged: pd.DataFrame, corr: float, pval: float, out_dir: str) -> None:
    ...
    fig.suptitle(
        f"달러인덱스(DXY) vs S&P 500 (Pearson r = {corr:.3f}  p {'< 0.001' if pval < 0.001 else f'= {pval:.4f}'})",
        color="white",
        fontsize=13,
        fontweight="bold",
    )
    ...
    ax1b = ax1.twinx()
    ax1b.fill_between(merged.index, merged["dxy"], alpha=0.25, color=BLUE)
    ax1b.plot(merged.index, merged["dxy"], color=BLUE, lw=1.2)
    ax1b.set_ylabel("달러인덱스 DXY (우축 역전)", color=BLUE)
    ax1b.tick_params(colors=GRAY)
    ax1b.invert_yaxis()
    ax1.set_title("S&P 500 & 달러인덱스 DXY — 시계열 (DXY 축 역전)", color=GREEN, fontsize=10)

    rc = merged["rolling_corr"].dropna()
    ax2.bar(rc.index, rc.values, color=[GREEN if v >= 0 else RED for v in rc], width=1, alpha=0.7)
    ax2.axhline(corr, color=CHART_BLUE, lw=1.5, ls="--", label=f"전체 r = {corr:.3f}")
    ax2.axhline(0, color=GRAY, lw=0.6)
    ax2.set_ylim(-1.1, 1.1)
    ax2.set_ylabel("Pearson r", color=GRAY)
    ax2.set_title("252일 롤링 상관계수", color=CHART_BLUE, fontsize=10)
    ax2.legend(framealpha=0.3, facecolor="#1a1a2e", labelcolor="white", fontsize=9)

    sc = ax3.scatter(
        merged["dxy"], merged["sp500"], c=merged.index.year, cmap="plasma", alpha=0.3, s=6
    )
    z = np.polyfit(merged["dxy"], merged["sp500"], 1)
    xs = np.linspace(merged["dxy"].min(), merged["dxy"].max(), 200)
    ax3.plot(xs, np.poly1d(z)(xs), color=CHART_BLUE, lw=2, ls="--", label=f"추세선 r = {corr:.3f}")
    ax3.set_xlabel("달러인덱스 DXY", color=GRAY)
    ax3.set_ylabel("S&P 500", color=GRAY)
    ax3.set_title("DXY vs S&P 500 산점도", color=CHART_BLUE, fontsize=10)
    ax3.legend(framealpha=0.3, facecolor="#1a1a2e", labelcolor="white", fontsize=9)
    plt.colorbar(sc, ax=ax3, label="연도")

    out = os.path.join(out_dir, "dxy_sp500_correlation.png")
...
def main() -> None:
    ...
    print("[dxy] DXY 데이터 다운로드...")
    dxy = fetch_dxy()
    print("[dxy] S&P 500 데이터 다운로드...")
    sp = fetch_sp500()

    merged = pd.concat([dxy, sp], axis=1).dropna()
    corr, pval = pearsonr(merged["dxy"], merged["sp500"])
    merged["rolling_corr"] = merged["dxy"].rolling(WINDOW).corr(merged["sp500"])
